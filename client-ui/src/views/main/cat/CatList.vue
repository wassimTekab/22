    ...
    <el-card shadow="never" class="card">
      <div class="header__wrapper">
        <div class="text-color">
          {{ $t("entityList.pagination.display") }} {{ pagination.total }}
      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/catList[scope.$index].id}`"
              >{{ catList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/edit/${catList[scope.$index].id}`"
              :underline="false"
              type="primary"
              class="me-3"
            >
            ...
            <el-button
              @click="() => handleOpenConfirmModal(catList[scope.$index]?.id)"
              icon="Delete"
              circle
              plain
              type="danger"
            ...
            <ConfirmModal
              ...
              :isOpenModal="isOpenModal && modalId === catList[scope.$index].id"
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              "
              @approve-confirm-modal="
                () => handleRemoveEntity(catList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "cats";
    const fieldsName = ref(["name", "color", "weight"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { catList, pagination, isLoading } = storeToRefs(useCatStore());
    const { fetchCats, deleteCat } = useCatStore();
